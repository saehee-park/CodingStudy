	...
	public static void main(String[] args) throws IOException {
		...
		Arrays.sort(result, (e1, e2) -> { // 람다식
			if(e1[0] == e2[0]) { // 첫번째 원소가 같다면 두번째 원소끼리 비교
				return e1[1] - e2[1]; // e1<e2면 음수값 | 같다면 0 | e1>e2면 양수값
			} else {                  // 두 원소의 대소 비교값을 쉽게 이해하기 위해 사용이 정보를 통해 어떤 원소가 
				return e1[0] - e2[0]; // 우선순위가 높은지 알아내고 두 원소이 위치를바꿀지말지 Arrays가 결정
			}
		});
