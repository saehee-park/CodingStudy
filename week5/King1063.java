...
public class King1063 {
	...
	public static void main(String[] args) throws IOException {
		...
		int kingX = row.indexOf(input[0].charAt(0)); // input[0] : 킹의 위치
		int kingY = col.indexOf(input[0].charAt(1));
		int stoneX = row.indexOf(input[1].charAt(0)); // input[1] : 돌의 위치
		int stoneY = col.indexOf(input[1].charAt(1));
		int test = Integer.parseInt(input[2]); // input[2] : 움직이는 횟수
		...
		while (test-- > 0) {
			...
			boolean condition = 0 <= moveKingX && moveKingX < 8 && 0 <= moveKingY && moveKingY < 8 && 0 <= moveStoneX
					&& moveStoneX < 8 && 0 <= moveStoneY && moveStoneY < 8;
